    ...
    P: AsRef<Path>,
{
    ...
    tokio::fs::create_dir(&items_dir)
        .await
        .map_err(|e| e.to_string())?;
    tokio::fs::create_dir(&thumbnail_dir)
        .await
        .map_err(|e| e.to_string())?;
    ...
    let file = File::create(custom_category_txt_path)
        .await
        .map_err(|e| e.to_string())?;
    ...
    let file = std::fs::File::create(items_data_json_path).map_err(|e| e.to_string())?;
    serde_json::to_writer(file, &avatar_explorer_items).map_err(|e| e.to_string())?;

    let file = File::create(common_avatar_json_path)
        .await
        .map_err(|e| e.to_string())?;
    let mut writer = tokio::io::BufWriter::new(file);
