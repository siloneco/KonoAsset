...
### 🚀新機能
- データ管理ダイアログでファイル名にホバーすることで全文が確認できるように
- サムネイルの手動追加でWebPなどを選択できるように
- OSのゴミ箱を利用するかどうかを設定で変更できるように
  - NASではゴミ箱が使用できないため、NASを利用する場合はゴミ箱を使用しない設定にしてください
- 重複確認のダイアログからスムーズに既存アセットへファイルをインポートできるように
- カテゴリ、対応アバター、タグについて、未指定のアセットを絞り込めるように

### 🐛バグ修正
- logsフォルダに9文字以下のファイルやフォルダがあると起動できない問題を修正
- クエリパラメータを含むBOOTH URLを受け入れられない問題を修正
- 特定の環境でサムネイルを手動選択できない問題を修正
- 統計タブの円グラフの合計値にその他のアセットが含まれていない問題を修正

### ⚙️その他
- 初期設定ページを改善
- サムネイルのインポート時にリサイズとJPEG化を行い、容量とレンダリング処理を軽量化
- 依存関係のアップデート

## 1.2.5

### 🚀新機能
- 対応アバターとタグ入力欄で改行を含むテキストを貼り付けたときに自動で入力が確定されるように

### 🐛バグ修正
- 情報入力でショップ名が正しく保存されないことがある問題を修正
- メモ入力欄が枠からはみ出て表示されることがある問題を修正
- カテゴリフィルタの候補にその他タイプのカテゴリが表示されない問題を修正

### ⚙️その他
- アセット名の表示行数を2行、ショップ名の表示行数を1行に制限するように変更

## 1.2.4

### 🚀新機能
- ウィンドウサイズや表示サイズ、表示順を再起動しても維持するように
- ファイルやフォルダ削除時にそれらをゴミ箱へ移動するように
